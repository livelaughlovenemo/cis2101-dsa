...
Node parent(Node n, const Tree *T) {
    ...
    // TODO
    if (n < 0 || n >= MAX_NODES) return INVALID_NODE;
    if (n == T->rootIndex) return ROOT_NODE;

    for (int i = 0; i < MAX_NODES; i++) {
        ChildNode *cur = T->children[i];
        while (cur) {
            if (cur->childIndex == n)
                return i;
            cur = cur->next;
...
Node left(const Tree *T) {
    ...
    // TODO
    for (int i = 0; i < MAX_NODES; i++) {
        // Skip if node has children or is not initialized
        if (T->children[i] != NULL) continue;

        // Check if node exists in the tree
        if (i == T->rootIndex) continue;

        // Check if node is in any parent's list
        bool exists = false;
        for (int j = 0; j < MAX_NODES; j++) {
            ChildNode *cur = T->children[j];
            while (cur) {
                if (cur->childIndex == i) {
                    exists = true;
                    break;
